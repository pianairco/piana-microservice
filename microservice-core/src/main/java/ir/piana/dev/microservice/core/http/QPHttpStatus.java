...
public enum QPHttpStatus {
    ...
    QPHttpStatus(int statusCode, String reasonPhrase) {
     ...
     *
     * @return {@link QPHttpStatus} representation of the status.
     */
    public static QPHttpStatus fromCode(final int statusCode)
            throws QPException {
        for(QPHttpStatus status : QPHttpStatus.values()) {
            if (status.statusCode == statusCode) {
                return status;
            }
        }
        throw new QPException("status not exist!");
    }
